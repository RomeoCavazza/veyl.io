...
export default function OEmbedDemo() {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const endpoint = `${apiBase}/api/v1/meta/oembed/public?url=${encodeURIComponent(url)}`;
      
      if (import.meta.env.DEV) {
        console.log('Fetching oEmbed from:', endpoint);
      }
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData?.detail?.message || errorData?.detail?.error_message || errorData?.detail || `HTTP ${response.status}`;
